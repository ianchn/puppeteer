...
class NavigatorWatcher {
  ...
  constructor(client, ignoreHTTPSErrors, options = {}) {
    ...
    console.assert(
      this._waitUntil === 'load' || 
      this._waitUntil === 'contentload' || 
      this._waitUntil === 'networkidle', 
      
      'Unknown value for options.waitUntil: ' + this._waitUntil);
  }
  ...
  async waitForNavigation() {
    ...
    } else if (this._waitUntil === 'contentload') {
      const domContentEventFired = new Promise(fulfill => {
        this._eventListeners.push(helper.addEventListener(this._client, 'Page.domContentEventFired', fulfill));
      }).then(() => null);
      navigationPromises.push(domContentEventFired);
    } else {
